...
def train():
    ...
    if args.dataset_type == 'blender':
        images_all, poses_start, render_poses, hwf, near, far, i_train, i_val, i_test = load_blender_data(args.datadir, near=args.near, far=args.far)
    ...
    images_val = torch.tensor(images_all[i_val])
    images_test = torch.tensor(images_all[i_test])

    poses_train = poses_start[i_train][:, :3, :4]
    poses_start_se3 = SE3_to_se3_N(poses_train)  # [34, 6]

    poses_end_se3 = poses_start_se3

    poses_org = poses_start.repeat(args.deblur_images, 1, 1)  # [29*7, 3, 5]
    ...
    test_poses_file = os.path.join(basedir, expname, 'test_poses.txt')
    ...
    pose_nums = torch.tensor([0, args.deblur_images - 1]).reshape(1, -1).repeat(1, 1)
    ...
    midpoints_viz = poses_start[:args.pic_num][:, :3, 3].detach().cpu().numpy()
    sidepoints_viz = poses_start[-(args.pic_num + 1):][:, :3, 3].detach().cpu().numpy()
    ...
    ax3d.scatter(midpoints_viz[: ,0], midpoints_viz[: ,1], midpoints_viz[: ,2], s=10, alpha=1, 
        c='g')
    ax3d.scatter(sidepoints_viz[: ,0], sidepoints_viz[: ,1], sidepoints_viz[: ,2], s=5, alpha=1, 
        c='b')
    ...
    ax3d.plot(sidepoints_viz[:, 0], sidepoints_viz[:, 1], sidepoints_viz[:, 2], color='b', linewidth=1)
    ...
    ### core optimization loop ###
        ...
        if args.rand_sample:
            assert False
            row_batch = torch.randperm(H)[:args.N_row]
            column_batch = torch.randint(0, W, (args.N_row, args.N_rowpoint))  # [64, 2]
        else:
        ...
        if i % args.i_img == 0 and i > 0 or i == 10000:
            ...
            img_dir = os.path.join(args.basedir, args.expname, 'img_test_in', str(i).zfill(5))
            with torch.no_grad():
                imgs_render = render_image_test(i, graph, all_poses[i_render], H, W, K, args, img_dir)
            mse_render = compute_img_metric(images_val, imgs_render, 'mse')
            psnr_render = compute_img_metric(images_val, imgs_render, 'psnr')
            ssim_render = compute_img_metric(images_val, imgs_render, 'ssim')
            lpips_render = compute_img_metric(images_val, imgs_render, 'lpips')
            
            with open(test_metric_file, 'a') as outfile:
                outfile.write(f"iter{i}: \n")
                outfile.write(f"test_in: MSE:{mse_render.item():.8f} PSNR:{psnr_render.item():.8f}"
