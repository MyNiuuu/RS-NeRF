...
class Model(nerf.Model):
    ...
    def build_network(self, args, special_iter=10000):
        self.graph = Graph(args, D=8, W=256, input_ch=63, input_ch_views=27, output_ch=4, skips=[4], use_viewdirs=True, special_iter=special_iter)
    ...
    def setup_nerf_optimizer(self, args):
        grad_vars = list(self.graph.nerf.parameters())
        if args.N_importance > 0:
            grad_vars += list(self.graph.nerf_fine.parameters())
        self.optim = torch.optim.Adam(params=grad_vars, lr=args.lrate, betas=(0.9, 0.999))
...
class Graph(nerf.Graph):
    def __init__(self, args, D=8, W=256, input_ch=63, input_ch_views=27, output_ch=4, skips=[4], use_viewdirs=True, special_iter=10000):
        super().__init__(args, D, W, input_ch, input_ch_views, output_ch, skips, use_viewdirs, special_iter)
    ...
    def get_pose(self, i, img_idx, args):
        ...
        se3_start = self.se3.weight[:, :6][img_idx]  # [29, 6]
        se3_end = self.se3.weight[:, 6:][img_idx]  # [29, 6]
        ...
        se3_start = se3_start[seg_pos_x, :]  # [29, 64, 6]
        se3_end = se3_end[seg_pos_x, :]  # [29, 64, 6]
